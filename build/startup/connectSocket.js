...
const {
  ...
  deleteImage,
  addImage
} = require('../wsActions/files');

const {
  deleteUser,
  logoutAll
} = require('../wsActions/user');
...
const {
  requestResource
} = require('../wsActions/requestResource');

const {
  saveUserSettings
} = require('../wsActions/saveUserSettings');
  ...
  try {
    ...
    wss.on('connection', function connection(ws) {
      ...
      ws.on('message', async message => {
        try {
          if (ws.createdTime + 5000 < Date.now()) await rateLimiter.consume(ws.user);
