...
userSchema.methods.createWebsite = async function () {
  try {
    ...
    const globalSettingsPage = await website.createGlobalSettingsResource('page');
    const globalSettingsTemplate = await website.createGlobalSettingsResource('template');
  ...
  try {
    ...
    if (websiteObject.user.toString() === this._id.toString()) {
      ...
      if (websiteObject.customDomain && websiteObject.customDomainApp) {
        await heroku.delete('/apps/' + websiteObject.customDomainApp + '/domains/' + websiteObject.customDomainId);
      }
    ...
    const index = this.websites.findIndex(item => item.id.toString() === _id.toString());
