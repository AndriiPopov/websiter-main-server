...
exports.refinePropertiesFromCMS = exports.default = void 0;
...
const toRGBAString = rgb => rgb ? `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${rgb.a})` : 'rgba(0,0,0,0)';

var _default = (props, elementValues) => {
  const result = {};
  ...
  return {
    refinedProperties: { ...props.parentPluginProps,
      ...result
    },
...
const refinePropertiesFromCMS = mD => {
  const addVariable = (mD, item, currentPageDraft, pageTemplateDraft, pluginId) => {
    ...
    if (itemValues.CMSVariableSystemName) if (itemValues.CMSVariableType === 'menuItems') {
      ...
      return {
        [itemValues.CMSVariableSystemName]: toRGBAString(resourceVariable && resourceVariable.color ? resourceVariable.color : itemValues.defaultColor)
      };
    } else if (itemValues.CMSVariableType.indexOf('propagating_') === 0 || itemValues.CMSVariableType === 'array') {
      const forPluginId = itemValues.CMSVariableType === 'array' ? pluginId : itemValues.CMSVariableType.slice('propagating_'.length);
      const forPluginDraft = mD.resourcesObjects[forPluginId];
      const propagatingValue = [];
      const children = currentPageDraft.structure.filter(el => (0, _isEqual.default)([...item.path, item.id], el.path));
      ...
      return {
        [itemValues.CMSVariableSystemName]: resourceVariable ? resourceVariable.value || itemValues.CMSVariableDefaultValue : itemValues.CMSVariableDefaultValue
      };
    }
  };
...
exports.refinePropertiesFromCMS = refinePropertiesFromCMS;