...
module.exports.addResource = async (data, ws) => {
  try {
    ...
    const {
      error
    } = addResourceSchema.validate(data);
    ...
    const result = await website.createResource(data._id, data.type, data.duplicate, data.resourceData, data.name);

    if (!result) {
      sendError(ws, 'New resource has not been created. Note, that you can not duplicate global settings resources.');
      return;
    }
...
const newWebsiteSchema = Joi.object({
  ...
  sharing: Joi.array().required().items(Joi.object({
    userId: Joi.objectId(),
    rights: Joi.array().items(Joi.string().valid('owner', 'admin', 'developer', 'content')),
    accountInfo: Joi.object().unknown()
  })),
  pagesStructure: Joi.array().required().items(Joi.object({
    id: Joi.objectId().required(),
    path: Joi.array().items(Joi.objectId()).required(),
  ...
  pluginsStructure: Joi.array().required().items(Joi.object({
    id: Joi.objectId().required(),
    path: Joi.array().items(Joi.objectId()).required(),
    name: Joi.string().allow(''),
    hidden: Joi.boolean(),
  ...
  templatesStructure: Joi.array().required().items(Joi.object({
    id: Joi.objectId().required(),
    path: Joi.array().items(Joi.objectId()).required(),
    name: Joi.string().allow(''),
    hidden: Joi.boolean(),
...
const newResourceSchema = Joi.object({
  ...
  structure: Joi.array().required().items(Joi.object({
    id: Joi.string().required(),
    path: Joi.array().required().items(Joi.string()),
...
module.exports.updateResource = async (data, ws) => {
  ...
  try {
    ...
    let {
      error
    } = updateResourceSchema.validate(data);
    ...
    if (data.type === 'website') {
      ...
      const newWebsite = diffpatcher.patch(website.toObject(), data.__patch__);
      let {
        error
      } = newWebsiteSchema.validate(newWebsite);
      ...
      if (!newWebsite.pagesStructure.find(item => item.generalSettings) || !newWebsite.templatesStructure.find(item => item.generalSettings)) {
        sendError(ws);
        return;
      }

      for (let attr in newWebsite) {
        if (!['id', 'domain', 'customDomain', 'customDomainApp', 'customDomainVerified', 'cname', 'verifyCode', 'user', 'storage', '__patch__'].includes(attr)) {
      ...
      const draft = resourceObject.draft.structure ? resourceObject.draft : resourceObject.published;
      const newResource = diffpatcher.patch(draft, data.__patch__);
      let {
        error
      } = newResourceSchema.validate(newResource);
      ...
      if (data.markPublish) {
        ...
        const resourceItem = website[structureType[data.type]].find(item => item.id.toString() === resource._id.toString());
...
module.exports.publishResource = async (data, ws) => {
  try {
    ...
    let {
      error
    } = publishResourceSchema.validate(data);
...
module.exports.revertResource = async (data, ws) => {
  ...
  try {
    ...
    let {
      error
    } = revertResourceSchema.validate(data);
    ...
    if (data.to === 'draft') {
      if (!resource.draft.structure) draft = resource.published;else draft = resource.draft;
    } else {
