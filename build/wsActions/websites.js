...
module.exports.addWebsite = async (data, ws) => {
  try {
    ...
    let website; // if (data.duplicate && data._id) {
    ...
    user.__patch__ = diffpatcher.diff(oldUserObject.websites, newUserObject.websites);
...
module.exports.deleteWebsite = async (data, ws) => {
  try {
    ...
    const oldUserObject = user.toObject();
    if (!(await getUserRights(user._id, data._id.toString(), 'owner', null, ws))) return;
...
module.exports.verifyCustomDomain = async (data, ws) => {
  try {
    ...
    if (!(await getUserRights(user._id, website, ['developer', 'admin'], null, ws))) return;
    ...
    if (txtVerified && cnameVerified) {
      ...
      for (let websiteInn of oldWebsites) {
        ...
        websiteInn.__patch__ = diffpatcher.diff(oldWebsiteInnObject, newWebsiteInnObject);
...
module.exports.addUserInSharing = async (data, ws) => {
  try {
    ...
    const {
      error
    } = addUserInSharingSchema.validate(data);
    ...
    const website = await Website.findById(data._id);
    if (!(await getUserRights(user._id, website, ['owner', 'admin'], null, ws))) return;
    ...
    if (data.type === 'add') {
      ...
      editUser.websites.push({
        id: website._id
      });
    } else {
      const index = editUser.websites.findIndex(item => item.id.toString() === website._id.toString());
      if (index >= 0) editUser.websites.splice(index, 1);
      website.sharing = website.sharing.filter(item => item.userId !== data.userId.trim());
    }
    ...
    editUser.__patch__ = diffpatcher.diff(oldUserObject.websites, newUserObject.websites);
...
module.exports.transferWebsite = async (data, ws) => {
  try {
    ...
    if (!(await getUserRights(user._id, website, ['owner'], null, ws))) return;
    const oldWebsiteObject = website.toObject();
    const oldUserObject = user.toObject();
    ...
    userTo.__patch__ = diffpatcher.diff(oldUserToObject.websites, newUserToObject.websites);
...
const saveDomainNameSchema = Joi.object({
  _id: Joi.objectId().required(),
  name: Joi.string().max(250).allow('').required(),
  type: Joi.string().valid('domain', 'customDomain').required()
}).unknown();
...
module.exports.saveDomainName = async (data, ws) => {
  try {
    ...
    const {
      error
    } = saveDomainNameSchema.validate(data);
    ...
    if (!(await getUserRights(ws.user.toString(), website, ['owner', 'admin', 'developer'], null, ws))) return;
    const oldWebsiteObject = website.toObject();
    ...
    if (data.type === 'domain') {
      ...
      if (website.customDomain.toLowerCase() !== oldWebsiteObject.customDomain.toLowerCase()) {
        if (website.customDomain !== '__delete__') {
          ...
          if (!website.customDomainApp || website.customDomainApp === '') {
            ...
            if (website.customDomainId) {
              try {
                await heroku.delete('/apps/' + website.customDomainApp + '/domains/' + website.customDomainId);
              } catch (ex) {}
            }
          }

          const addDomainData = await heroku.post('/apps/' + website.customDomainApp + '/domains', {
            body: {
