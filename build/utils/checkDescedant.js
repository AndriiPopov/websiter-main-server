...
const {
  pure
} = require('./testPopulateDBandReadStructure');

const {
  structureType
} = require('./resourceTypeIndex');
...
module.exports.resourceIsInWebsite = async (resourceId, websiteId, type) => {
  ...
  if (!website) {
    ...
    if (!website[structureType[type]].some(resource => resource.id.toString() === resourceId)) {
      return false;
    }
  }
...
module.exports.resourceIsInUser = async (resourceId, user, res) => {
  ...
  if (!user.websites.some(website => website.id.toString() === resource.website.toString())) {
