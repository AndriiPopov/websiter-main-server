...
module.exports.getUserRights = async (id, website, right, res, ws) => {
  try {
    ...
    if (!result) {
      if (ws) sendError(ws, 'Not enough rights.');else if (res) res.status(412).send('Not enough rights.');
    }
...
const checkUserRights = async (id, website, right, res, ws) => {
  try {
    ...
    let owner = website.sharing.find(item => item.userId.toString() === website.user.toString());
    ...
    if (!owner) {
      ...
      await website.save();
    } else if (!owner.rights.includes('owner') || !owner.rights.includes('admin')) {
      owner = {
        userId: website.user,
        rights: ['owner', 'admin']
      };
    ...
    const account = website.sharing.find(item => item.userId.toString() === id.toString());
    ...
    if (account) {
      if (!right) {
        ...
        if (!allow) return false;else return true;
      }
    }
