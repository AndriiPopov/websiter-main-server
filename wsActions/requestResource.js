...
module.exports.requestResource = async (data, ws) => {
    try {
        ...
        if (data.id && data.type) {
            ...
            if (data.type === 'user') resource = await User.findById(data.id)
            else if (data.type === 'website')
                resource = await Website.findById(data.id)
            else if (data.type === 'resource')
                resource = await Resource.findById(data.id)
            if (resource) {
                if (data.type === 'resource') {
                    resource = {
                        draft: !resource.draft.structure
                            ? resource.published
                            : resource.draft,
                        present: {},
                        future: [],
